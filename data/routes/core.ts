 ...
 * fields — tier components, party capacity, guide links — are layered on in
 * `data/destinations.ts` from `data/routes/commerce.ts`, so the published copy
 * here never has to change when the commercial model does.
 */
export const coreDestinations: DestinationContent[] = [
  {
